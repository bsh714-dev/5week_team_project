...
describe('Layered Architecture Pattern Posts Service Unit Test', () => {
  ...
  test('Posts Service createPost Method By Success', async () => {
    ...
    // 1. userId, nickname, title, content 이용해 게시글을 생성한다.(PostRepository.createPost)
    ...
    expect(mockPostsRepository.createPost).toHaveBeenCalledWith(
      ...
      createPostData.nickname,
      createPostData.title,
      createPostData.content
    );
    // 2. createPost Method의 Return 값이 내가 원하는 형태인지 확인한다.
  ...
  test('Posts Service updatePost Method By Success', async () => {
    ...
    // Mock Post Repositorydml findPostById Method의 Return 값을 findPostByIdReturnValue 변수로 변경합니다.
    mockPostsRepository.findPostById = jest.fn(() => {
      return findPostByIdReturnValue;
    });

    const updatePost = await postService.updatePost(1, 'Title_1', 'Content_1');
    ...
    expect(mockPostsRepository.updatePost).toHaveBeenCalledWith(
      findPostByIdReturnValue.postId,
      findPostByIdReturnValue.title,
      findPostByIdReturnValue.content
    );
  ...
  test('Posts Service deletePost Method By Success', async () => {
    ...
    // 2. postId 이용해 게시글을 삭제한다. (PostRepository.deletePost)
  ...
  test('Posts Service deletePost Method By Not Found Post Error', async () => {
    ...
    try {
      const deletePost = await postService.deletePost(8888);
    } catch (error) {
      // 1. postId를 이용해 게시글을 찾고 (PostRepository.findPostById)
      expect(mockPostsRepository.findPostById).toHaveBeenCalledTimes(1);
